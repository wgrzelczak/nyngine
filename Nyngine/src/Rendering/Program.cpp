...
namespace ny::Rendering
{
    ...
    {
        u32 attribsNum{0};
        glGetProgramiv(m_programId, GL_ACTIVE_ATTRIBUTES, reinterpret_cast<GLint*>(&attribsNum));
    ...
    {
        u32 uniformsNum{0};
        glGetProgramiv(m_programId, GL_ACTIVE_UNIFORMS, reinterpret_cast<GLint*>(&uniformsNum));
    ...
    {
        if (int loc = GetUniformLocation(location); loc >= 0)
            glUniform1i(loc, value);
    }

    void Program::SetUint(std::string location, u32 value) const
    {
        if (int loc = GetUniformLocation(location); loc >= 0)
