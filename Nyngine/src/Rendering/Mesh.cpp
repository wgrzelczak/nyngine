...
namespace ny::Rendering
{
    ...
    void Mesh::Generate(VertexAttributesSettings settings)
    {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
        glBufferData(GL_ARRAY_BUFFER, settings.m_positionsNum * settings.m_positionsStride * 3, settings.p_positions, GL_STATIC_DRAW);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, settings.m_positionsStride * 3, (void*)(0));
        glEnableVertexAttribArray(0);

        m_indicesNum = settings.m_indicesNum;
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ebo);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, settings.m_indicesNum * settings.m_indicesStride, settings.p_indices, GL_STATIC_DRAW);
